#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
int main(void) {
	int fd[2];
	char buf[20], line[20] = "hello grandchild\n";
	pid_t pid, pid2;
	int c;

	if (pipe(fd) < 0)
		perror("pipe error");
	if ((pid = fork()) < 0) //first fork
		perror("fork error");
	else if (pid > 0) { //parent
		close(fd[0]); //parent is not reading from pipe
		write(fd[1], line, 20); //parent is writing in pipe
	}
	else {
		if ((pid2 = fork()) < 0) //second fork
			perror("second fork error");
		else if (pid2 == 0) { //grandchild
			close(fd[1]); //grandchild is not writing in pipe
			c = read(fd[0], line, 20); //grandchild is reading from pipe
			buf[c] = '\0';
			printf("%s\n", line);
		}
		exit(0);
	}
}
